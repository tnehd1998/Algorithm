...
function setParent(parentArr, pointA, pointB) {
  let parentA = getParent(parentArr, pointA);
  let parentB = getParent(parentArr, pointB);
...
function solution(n, costs) {
  ...
  for (let i = 0; i < costs.length; i++) {
    ...
    if (
      getParent(parentArr, currentCost[0]) !==
      getParent(parentArr, currentCost[1])
    ) {
      answer += currentCost[2];
      setParent(parentArr, currentCost[0], currentCost[1]);
    }
  }
