...
function timeToSecond(time) {
  let times = time.split(":");
  let hour = Number(times[0]) * 3600;
  let minute = Number(times[1]) * 60;
...
function solution(play_time, adv_time, logs) {
  let totalTime = timeToSecond(adv_time);
  let endTime = timeToSecond(play_time);

  let timeArr = Array(endTime).fill(0);

  for (let i = 0; i < logs.length; i++) {
    let currentTime = logs[i].split("-");
    let startTime = timeToSecond(currentTime[0]);
    let finishTime = timeToSecond(currentTime[1]);
    timeArr[startTime]++;
    timeArr[finishTime]--;
  }
  ...
  let answer = 0;
  let highestCost = timeArr[totalTime - 1];

  for (let i = totalTime - 1; i < endTime; i++) {
    let currentPeople = timeArr[i] - timeArr[i - totalTime];
    if (highestCost < currentPeople) {
      highestCost = currentPeople;
      answer = i - totalTime + 1;
    }
  }
